...
const AddAmount = () => {
  ...
  // Extract unique mosque from tasks data
  // Extract unique mosque from tasks data and filter out empty strings
  const availableYears = Array.from(
    new Set(
      tasks?.mosque?.map((year) => year.year).filter((year) => year !== "")
    )
  );
        ...
        <div style={{ marginTop: "100px" }}>
          <Card>
            <CardContent>
              ...
              {tasks?.mosque?.map(
                (yearData) =>
                  // Apply year filter if selected, or display all mosque
